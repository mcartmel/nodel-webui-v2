...
export async function verifyDependencies(options = {}) {
  ...
  const audit = await verifyAudit({ projectRoot, outputDir, exceptionsPath: options.exceptionsPath }); return { lockHash: evidence.lockHash, sbom: evidence.sbom, licenses: evidence.licenses, audit };
}
