...
The development catalogue is organised by authoring destination: Quickstart, App & Layout, Controls, Display & Content, and Guides. Each component page owns its component reference and keeps copyable markup free of catalogue wrappers or runtime metadata. Guides explain shared behaviour such as sizing, visibility, actions, dynamic options, appearance, and confirmations and link to the relevant component pages without duplicating references. Responsive spans stay with Rows & Columns, responsive control-grid counts stay with Control Grid & Spacer, and backgrounds stay with App. This catalogue is development-only and uses clean title-derived routes without legacy aliases.
