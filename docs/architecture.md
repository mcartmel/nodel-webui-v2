...
Node editor behavior intentionally preserves the v1 file endpoints while using CodeMirror 6 rather than CodeMirror 5. The editor shell, file browser, controls, and status state are JsViews-linked. CodeMirror owns only the editor viewport. Structured component attributes, values, defaults, and constraints are maintained in `src/nodel-component-metadata.ts`; `src/editor/nodel-document-definition.ts` adapts that shared metadata into CodeMirror completions. The metadata should be updated whenever a public `nodel-*` component is added or changed.
