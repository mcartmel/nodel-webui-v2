...
The runtime targets the tested Java Nodel API directly and does not perform generic feature negotiation. The release manifest schema is version 4 and records the supported API contract range, release-process CI/environment provenance, the canonical `dist` inventory digest, the hash-bound component-contract descriptor, and normalized hash-pinned Java handoff reports. This is a project-owned version for the source-backed REST/WebSocket contract recorded in `test/fixtures/java-nodel-api.json`, not the Java host's `nodelVersion` and not a value negotiated at runtime. A consuming Java build maps its implementation to this contract during packaging. Alternative-backend negotiation can be designed later from a concrete cross-backend requirement.
